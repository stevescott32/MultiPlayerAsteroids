...
//
// --------------------------------------------------------------
MyGame.renderer.PowerUp = function(spec, graphics) {
    'use strict'; 
    ...
    //
    //------------------------------------------------------------------
    function render(model, texture) {
        if (isReady) {

        graphics.saveContext();
        graphics.rotateCanvas(model.position, 0, canvases);
        graphics.drawSubTexture(texture, subImageIndex, subTextureWidth, model.position, model.size, canvases);
